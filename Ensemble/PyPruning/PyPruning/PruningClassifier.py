...
#Source: https://github.com/sbuschjaeger/PyPruning/blob/master/PyPruning/PruningClassifier.py
#Not adapted 
class PruningClassifier(ABC): 
    ...
    def prune_(self, proba, target, data = None):
        """
        Prunes the ensemble using the ensemble predictions proba and the pruning data targets / data. 
        If the pruning method requires access to the original ensemble members you can access these via self.estimators_. 
        Note that self.estimators_ is already a deep-copy of the estimators so you are also free to change the estimators in this list if you want to.

        Parameters
        ----------
        proba : numpy matrix
            A (M,N,C) matrix which contains the individual predictions of each ensemble member on the pruning data. 
            Each ensemble prediction is generated via predict_proba. N is size of the pruning data, M the size of the 
            base ensemble and C is the number of classes
    ...
    def prune(self, proba, target, estimators, n_classes = None):
        """
        ...
        idx, weights = self.prune_(proba, target)        
        estimators_ = []
        for i in idx:
